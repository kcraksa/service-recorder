		...
		<div class="row">
			<div class="col-12">
				<div class="card">
					...
					<div class="card-body">
						<form method="post" action="<?php echo base_url('masterkendaraan/update/'.$detail->id) ?>">
							<div class="mb-3">
							  	...
							  	<select class="form-select" aria-label="Default select example" name="jenis_id" id="jenis_id">
								  	...
								  	<?php foreach($data_jenis_kendaraan as $jenis_kendaraan): ?>
								  		<?php 
								  			...
								  			$selected = $jenis_kendaraan->id == $detail->jenis_id ? 'selected' : '';

								  		?>
								  		<option value="<?php echo $jenis_kendaraan->id ?>" <?php echo $selected ?>><?php echo $jenis_kendaraan->jenis ?></option>
								  	<?php endforeach ?>
								</select>
							...
							<div class="mb-3">
							  	...
							  	<input type="text" name="nama" id="nama" class="form-control" value="<?php echo $detail->nama_kendaraan ?>">
							  	<?php echo form_error('nama'); ?>
							</div>
							<div class="mb-3 text-end">
							  	<button class="btn btn-primary" type="submit" onclick="javascript:confirm('Anda Yakin ?')">Update</button>
							  	<a class="btn btn-danger" href="<?php echo base_url('masterkendaraan') ?>">Cancel</a>
							</div>
						</form>
					</div>
				</div>
